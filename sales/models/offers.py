...
class Offer(models.Model):
    ...
    status = models.CharField(max_length=20,
                              ...
                              default='sent')
    offer_note = models.TextField(max_length=1500,
                                  null=True,
                                  blank=True)
    ...
    def save(self, *args, **kwargs):
        if not self.offer_number:
            ...
            last_offer = Offer.objects.filter(offer_number__startswith=f'O-{current_year_month}').order_by(
                '-offer_number').first()
            if last_offer:
                last_offer_number = int(last_offer.offer_number.split('-')[2])
            else:
                last_offer_number = 0
            self.offer_number = f'O-{current_year_month}-{str(last_offer_number + 1).zfill(3)}'
